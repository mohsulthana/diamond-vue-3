  ...
  <div class="p-col-12">
    <div class="card">
      ...
      <DataTable :value="customer1" :paginator="true" class="p-datatable-gridlines" :rows="10" dataKey="id" :rowHover="true" v-model:filters="filters1" filterDisplay="menu" :loading="loading1" :filters="filters1" responsiveLayout="scroll" :globalFilterFields="['name','country.name','representative.name','balance','status']">
        ...
        <Column header="Status" filterField="country.name" style="min-width:12rem">
          <template #body="{data}">
            <span class="p-column-title">Status</span>
            <span style="margin-left: .5em; vertical-align: middle" class="image-text">{{data.status}}</span>
          </template>
          <template #filter="{filterModel}">
        ...
        <Column header="Description" filterField="description" :showFilterMatchModes="false" :filterMenuStyle="{'width':'14rem'}" style="min-width:14rem">
          <template #body="{data}">
        ...
        <Column header="Employee" filterField="date" dataType="date" style="min-width:10rem">
          <template #body="{data}">
            <RadioButton id="option1" name="option" :value="true" v-model="data.verified" />
          </template>
          <template #filter="{filterModel}">
